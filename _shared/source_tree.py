...
def _ensure_submodules(
    ...
    *,
    ...
    if not submodules:
        return
    _run_git(
        _git_in_tree(source_root, "submodule", "update", "--init", "--depth", "1", "--", *submodules),
...
def ensure_source_tree(
    ...
    *,
    ...
    if any(not str(path).strip() for path in submodules):
        raise ValueError("submodules must contain only non-empty paths")

    override = os.environ.get(env_var)
    if override:
        source_root = Path(override).expanduser().resolve()
        _ensure_submodules(source_root, submodules, repository=repository, commit=commit)
        return source_root
    ...
    with _cache_lock(source_root):
        ...
        try:
            ...
            _ensure_submodules(temporary, submodules, repository=repository, commit=commit)
