...
def source_path() -> Path:
    ...
    ensure_shared_utils_submodule(source)
    return source


def ensure_shared_utils_submodule(source: Path) -> None:
    util_py = source / "proteinbert" / "shared_utils" / "util.py"
    if util_py.is_file():
        return
    subprocess.run(
        ["git", "-C", str(source), "submodule", "update", "--init", "--depth", "1", "proteinbert/shared_utils"],
        check=True,
    )
...
def main() -> None:
    ...
    if case != CASE:
        raise SystemExit(f"Unknown ProteinBERT case {case!r}; expected {CASE!r}")
