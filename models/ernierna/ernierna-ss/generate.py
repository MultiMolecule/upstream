...
def upstream_forward(sequence: str) -> dict[str, torch.Tensor]:
    ...
    with torch.no_grad():
        ...
        logits_ss = post_process(raw_logits, data_seq, 0.01, 0.1, 100, 1.6, True, 1.5)
        logits_ss = logits_ss.clamp(0, 1)
        logits_ss = logits_ss.unsqueeze(-1).contiguous()
    return {"logits_ss": logits_ss}
