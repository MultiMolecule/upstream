...
def parse_case(case: str) -> OpenSpliceAiCase:
    match = re.fullmatch(r"openspliceai-(?P<species>[a-z]+)-(?P<context>\d+)nt", case)
    if match is None:
...
def main() -> int:
    ...
    parser.add_argument("case", help="OpenSpliceAI fixture case, for example openspliceai-mane-80nt.")
