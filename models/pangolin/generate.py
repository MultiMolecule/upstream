...
def upstream_forward(sequence: str, source_root: Path, model_root: Path) -> torch.Tensor:
    ...
    # Pangolin's heads apply softmax (splice-site, 2 channels) and sigmoid (usage, 1 channel) per tissue,
    # so the upstream output is post-activation probabilities, not pre-activation logits.
    ...
    with torch.no_grad():
        for tissue_index, group in enumerate(ENSEMBLE_MEMBERS):
            ...
            probabilities = torch.stack(member_outputs).mean(dim=0)
            start = tissue_index * 3
            tissue_probabilities.append(probabilities[..., start : start + 3])
    return torch.cat(tissue_probabilities, dim=-1).contiguous()
...
def main_for_case(case_name: str) -> None:
    ...
    expected = {"probabilities": upstream_forward(sequence, source_root, model_root)}
    ...
    print(f"  probabilities: {tuple(expected['probabilities'].shape)}")
