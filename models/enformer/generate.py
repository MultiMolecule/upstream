...
def upstream_forward(sequence: str, saved_model: Path) -> np.ndarray:
    ...
    # The official head applies softplus, so this is post-activation coverage, not pre-activation logits.
    coverage = tf.gather(outputs["human"], TARGET_SLICE, axis=-1).numpy()
...
def main() -> None:
    ...
    expected = {"coverage": upstream_forward(sequence, saved_model)}
